...
contract SupplyChain {
  ...
  function harvestItem(uint _upc, address _originFarmerID, string memory _originFarmName, string memory _originFarmInformation, string memory _originFarmLatitude, string memory _originFarmLongitude, string memory _productNotes) public 
  {
    ...
    Item memory _item = Item(sku, _upc, msg.sender, _originFarmerID, _originFarmName, _originFarmInformation, _originFarmLatitude, _originFarmLongitude, 
    _upc+sku, _productNotes, 0, defaultState, address(0), address(0), address(0));
  ...
  itemState = getItemStateOrdinal(_item.itemState),
  ...
  function getItemStateOrdinal(State _itemState) private pure returns(uint state) {
    if (_itemState == State.Harvested) {
      state = 0;
    } else if (_itemState == State.Processed) {
      state = 1;
    } else if (_itemState == State.Packed) {
      state = 2;
    } else if (_itemState == State.ForSale) {
      state = 3;
    } else if (_itemState == State.Sold) {
      state = 4;
    } else if (_itemState == State.Shipped) {
      state = 5;
    } else if (_itemState == State.Received) {
      state = 6;
    } else {
      state = 7;
    }
